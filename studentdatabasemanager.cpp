...
QList<SubjectPerformanceDetails> StudentDatabaseManager::getSemesterDetails(QString roll, int sem)
{
    if(db.open())
    {
        QList<SubjectPerformanceDetails> list;
        QString queryString = QString("SELECT * FROM student_").append(roll.toLower()).append(QString(" WHERE Semester = %1").arg(sem));
        QSqlQuery query = db.exec(queryString);
        while(query.next())
        {
            list.append(SubjectPerformanceDetails(query.value(0).toString(), query.value(1).toInt(), query.value(2).toInt(), query.value(3).toString()));
        }
        db.close();
        return list;
    }
}
float StudentDatabaseManager::generateSGPA(QString roll , int sem)
{
    if(db.open())
    {
        float num = 0, den =  0;
...
float StudentDatabaseManager::generateCGPAuptoSem(QString roll, int sem)
{
    if(db.open())
    {
        float num = 0, den =  0;
